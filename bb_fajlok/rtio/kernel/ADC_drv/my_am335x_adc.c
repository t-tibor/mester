 ...
 *
 * New features:
 * 	- Step trigger can be set to hw from device tree.
 *	- Configurable HW_preemption, hw_event mapping, clock division.
 * 
 */
...
#define TRG_MODE_SW_ONE		0
#define TRG_MODE_SW_CONT	1	
#define TRG_MODE_HW_ONE		2
#define TRG_MODE_HW_CONT	3

// register bits extension
#define CNTRLREG_HW_EVT_MAP	(1<<8)
#define CNTRLREG_HW_PREEMPT	(1<<9)
...
static void tiadc_step_config(struct iio_dev *indio_dev)
{
	...
	for (i = 0; i < adc_dev->channels; i++) {
		...
		{
			// modified
			if(adc_dev->trigger_mode[i] > 3)
			{
				dev_warn(dev, "chan %d trigger mode reset to %d\n", chan, TRG_MODE_SW_CONT);
				adc_dev->trigger_mode[i] = TRG_MODE_SW_CONT;
			}

			stepconfig |= STEPCONFIG_MODE(adc_dev->trigger_mode[i]);
			  ...
			  struct tiadc_device *adc_dev)
{
	...
	// modified
	adc_dev->hw_preempt = 0;
	of_property_read_u32(node,"ext,hw-preempt", &adc_dev->hw_preempt);
	adc_dev->hw_event_map = 0;
	of_property_read_u32(node,"ext,hw-event-map",&adc_dev->hw_event_map);
	adc_dev->adc_clkdiv = 0;
	of_property_read_u32(node,"ext,adc-clkdiv",&adc_dev->adc_clkdiv);
	// value check
	if(adc_dev->hw_preempt > 1)
	{
		dev_warn(&pdev->dev,"Invalid hw preempt value: %u. Resetting it to 0.",adc_dev->hw_preempt);
		adc_dev->hw_preempt = 0;
	}
	if(adc_dev->hw_event_map > 1)
	{
		dev_warn(&pdev->dev,"Invalid hw event map value: %u. Resetting it to 0.",adc_dev->hw_event_map);
		adc_dev->hw_event_map = 0;
	}
	if((adc_dev->adc_clkdiv & 0xFFFF0000) > 0) // clk div should be max 2^16-1 big
	{
		dev_warn(&pdev->dev,"Invalid adc clkdiv value: %u. Resetting it to 0.",adc_dev->adc_clkdiv);
		adc_dev->adc_clkdiv = 0;
	}
	// modified END
	...
	of_property_for_each_u32(node, "ti,adc-channels", prop, cur, val) {
		...
		adc_dev->trigger_mode[channels] = TRG_MODE_SW_CONT;
	...
	// modified
	of_property_read_u32_array(node, "ext,trigger-mode",
				   adc_dev->trigger_mode, channels);
	// modified END
...
static int tiadc_probe(struct platform_device *pdev)
{
	...
	// modified
	regval = tiadc_readl(adc_dev, REG_CTRL);
	if(adc_dev->hw_preempt == 1)
		regval |= CNTRLREG_HW_PREEMPT;
	if(adc_dev->hw_event_map == 1)
		regval |= CNTRLREG_HW_EVT_MAP;
	tiadc_writel(adc_dev, REG_CTRL, regval);

	tiadc_writel(adc_dev, REG_CLKDIV, adc_dev->adc_clkdiv);
	// modified END
...
MODULE_DESCRIPTION("Modified TI ADC controller driver");
MODULE_AUTHOR("Tibor Tusori");
MODULE_LICENSE("GPL");
